...
```
## 探索发现
以上就几种常见的后台服务保活常用的解决方案，通过测试发现，Activity保活，Notication保活，双Services守护，SyncAdapter、JobService，循环播放音乐，这些方案不管是明手段还是流氓手段在android5.0下都还算好使，但是在Android6.0以后不管是哪种方案，在app进入后台，亮屏情况下好用，如果手机黑屏情况下，顶多半个小时进程就被系统杀死了，为此我几乎把全网关于Android后台进程保活的文章都看了一遍，方案都大同小异，无外乎就上面所罗列的几种方案。
<br><br>那么在6.0以后应用进入后台后，当终端处于黑屏模式，也就是Doze模式时，系统到底进行了哪些操作：
...
android6.0以后google对系统安全这块的保护措施更进一步增强了，程序进入后台系统为了省电，就认为该应用用户已经不再使用了，系统就自动清理掉该进程，有的人可能会问微信，支付宝这些应用为什么用户杀都杀不掉，原因就在于他们已经进入了系统进程白名单了如果你不信你可以去自己看看。
设置白名单有两种方式，一种就是手机厂商自行设置，另外一种就需要用户进入手机省电模式自己进行手动设置。
<br><br>所以最终结论: 在7.0这个版本，如果不通过用户或厂家设置，至少service是绝对没有任何办法保活的，绝对，除非你还能找到未知的BUG。进入进程白名单，然后通过双服务保活，加上JobService定时任务保驾护航，就可以解决问题了。但我真的很赞同谷歌这样的做法，如果每个应用都是后台常驻，整个系统的性能将大打折扣，都说android 手机耗电，现在android可以说是将一切统统杀掉，然后最后的决定权留给用户。
## 问题反馈
